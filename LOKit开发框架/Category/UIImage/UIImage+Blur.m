...
- (UIImage *)imageBlur:(float)blur {
    return  [self blurryImage:self withBlurLevel:blur];
}
...
- (UIImage *)imageBlurWithRadius:(CGFloat)blurRadius
    ...
    if (self.size.width < 1 || self.size.height < 1) {
        LOLogError (@"*** error: invalid size: (%.2f x %.2f). Both dimensions must be >= 1: %@", self.size.width, self.size.height, self);
        return nil;
    }
    if (!self.CGImage) {
        LOLogError (@"*** error: image must be backed by a CGImage: %@", self);
        return nil;
    }
    if (maskImage && !maskImage.CGImage) {
        LOLogError (@"*** error: maskImage must be backed by a CGImage: %@", maskImage);
        return nil;
    }
    ...
    if (hasBlur || hasSaturationChange) {
        ...
        if (hasBlur) {
            ...
            unsigned int radius = (unsigned int)floor(inputRadius * 3. * sqrt(2 * M_PI) / 4 + 0.5);
            if (radius % 2 != 1) {
...
- (UIImage *)imageAddTintEffectWithColor:(UIColor *)tintColor {
    ...
    return [self imageBlurWithRadius:10
...
- (UIImage *)imageAddLightEffect {
    ...
    return [self imageBlurWithRadius:2
                           tintColor:tintColor
...
- (UIImage *)imageAddExtraLightEffect {
    ...
    return [self imageBlurWithRadius:4
                           tintColor:tintColor
...
- (UIImage *)imageAddDarkEffect {
    UIColor *tintColor = [UIColor colorWithWhite:0.11 alpha:0.83];
    return [self imageBlurWithRadius:2
                           tintColor:tintColor
               saturationDeltaFactor:1.8
                           maskImage:nil];
